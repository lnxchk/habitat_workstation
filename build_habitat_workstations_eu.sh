...
for host in $(aws ec2 run-instances --image-id $AMI_ID --region eu-west-1 --count $NUMHOSTS --instance-type $INSTANCE_TYPE --key-name $KEY_NAME --security-group-ids "sg-b2088ed5" --subnet-id subnet-94827ff1 | jq -r ".Instances|.[].InstanceId"); do
